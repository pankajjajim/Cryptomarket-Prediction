...
function buildChartData(price, change24h, volume, timeframe) {
  ...
  volBars.forEach(function(v, idx) { volBars[idx] = (v / sum) * (volume || 1); });
...
function renderMarketChart(price, change24h, volume, symbol, chartTimeframe) {
  var chartW = 560, priceH = 200, volH = 56, totalH = priceH + volH;
  var leftPad = 48, rightPad = 52, topPad = 12, bottomPad = 8;
  var plotW = chartW - leftPad - rightPad, plotPriceH = priceH - topPad - bottomPad, plotVolH = volH - 4;
  var data = buildChartData(price, change24h, volume, chartTimeframe || '24h');
  var priceVals = data.priceVals, minP = data.minP, maxP = data.maxP, volBars = data.volBars;
  var priceRange = maxP - minP || price * 0.01;
  var maxVol = Math.max.apply(null, volBars.concat([1]));

  var pathD = priceVals.map(function(v, i) {
  ...
  var timeLabels = chartTimeframe === '24h' ? ['12 AM', '4 AM', '8 AM', '12 PM', '4 PM', '8 PM', '12 AM'] : chartTimeframe === '1W' ? ['Mon', 'Wed', 'Fri', 'Sun'] : ['Week 1', 'Week 2', 'Week 3', 'Week 4'];
  ...
  volBars.forEach(function(v, i) {
    ...
    volRects += '<rect x="' + x.toFixed(1) + '" y="' + y.toFixed(1) + '" width="' + barW.toFixed(1) + '" height="' + barH.toFixed(1) + '" fill="#1e3a5f" rx="1"/>';
  });

  var gridLines = [0.25, 0.5, 0.75].map(function(f) {
    var y = topPad + plotPriceH * (1 - f);
    return '<line x1="' + leftPad + '" y1="' + y.toFixed(1) + '" x2="' + (leftPad + plotW) + '" y2="' + y.toFixed(1) + '" stroke="#374151" stroke-width="0.5" stroke-dasharray="4"/>';
  }).join('');

  var yLabels = yTicks.map(function(p) {
    var y = topPad + plotPriceH - ((p - minP) / priceRange) * plotPriceH;
    return '<text x="' + (leftPad + plotW + 6) + '" y="' + (y + 4) + '" fill="#9ca3af" font-size="10" font-family="system-ui">' + formatChartPrice(p) + '</text>';
  }).join('');

  var currentY = topPad + plotPriceH - ((price - minP) / priceRange) * plotPriceH;
  var priceLabel = '<g><rect x="' + (leftPad + plotW - 44) + '" y="' + (currentY - 10) + '" width="42" height="18" rx="4" fill="' + lineColor + '" opacity="0.9"/><text x="' + (leftPad + plotW - 23) + '" y="' + (currentY + 2) + '" fill="#fff" font-size="10" font-weight="bold" text-anchor="middle">' + formatChartPrice(price) + '</text></g>';

  var xLabels = timeLabels.map(function(label, i) {
    var x = leftPad + ((i + 0.5) / timeLabels.length) * plotW;
    return '<text x="' + x + '" y="' + (priceH + volH - 2) + '" fill="#6b7280" font-size="9" text-anchor="middle">' + label + '</text>';
  }).join('');
  ...
  return '<div class="rounded-lg overflow-hidden bg-gray-800 border border-gray-700">' +
    ...
    '<span class="chart-timeframe px-2 py-1 rounded text-xs cursor-pointer ' + tf24 + '" data-tf="24h">24h</span>' +
    '<span class="chart-timeframe px-2 py-1 rounded text-xs cursor-pointer ' + tf1W + '" data-tf="1W">1W</span>' +
    '<span class="chart-timeframe px-2 py-1 rounded text-xs cursor-pointer ' + tf1M + '" data-tf="1M">1M</span>' +
    '<span class="chart-timeframe px-2 py-1 rounded text-xs cursor-pointer ' + tf1Y + '" data-tf="1Y">1Y</span>' +
    '</div></div>' +
    '<div class="p-3"><svg viewBox="0 0 ' + chartW + ' ' + totalH + '" class="w-full" style="max-height: 280px;">' +
    gridLines + '<path d="' + pathD + '" fill="none" stroke="' + lineColor + '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>' +
    priceLabel + volRects + yLabels + xLabels +
    '</svg></div></div>';
}
